...
//wwh
class VertexPose3DoF : public g2o::BaseVertex<3, Eigen::Vector3d>
{
   // Translation is the only optimizable variables
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    VertexPose3DoF(){}
    VertexPose3DoF(KeyFrame* pKF);
    ...
    virtual void oplusImpl(const double* update_){
        Eigen::Vector3d ut;
        ut << update_[0], update_[1], update_[2];
        setEstimate(estimate()+ut);
    }
};
...
// wwh
class  EdgeSE3ProjectXYZOnlyPose2: public  g2o::BaseUnaryEdge<2, Eigen::Vector2d, VertexPose3DoF>{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    EdgeSE3ProjectXYZOnlyPose2(){}
    ...
    void computeError()  {
        ...
        Eigen::Vector3d Xc = Rcw * Xw + v1->estimate();
        _error = obs-pCamera->project(Xc);
    }

    bool isDepthPositive() {
        const VertexPose3DoF* v1 = static_cast<const VertexPose3DoF*>(_vertices[0]);
        return (Rcw.row(2) * Xw + v1->estimate()(2)) > 0.0;
    }
...
class  EdgeStereoSE3ProjectXYZOnlyPose2: public  g2o::BaseUnaryEdge<3, Eigen::Vector3d, VertexPose3DoF>{
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  EdgeStereoSE3ProjectXYZOnlyPose2(){}
