...
//wwh
VertexPose3DoF::VertexPose3DoF(KeyFrame* pKF)
{
    setEstimate(pKF->GetTranslation().cast<double>());
}
...
//wwh
bool EdgeSE3ProjectXYZOnlyPose2::read(std::istream& is){
    for (int i=0; i<2; i++){
        is >> _measurement[i];
    }
    for (int i=0; i<2; i++)
...
bool EdgeSE3ProjectXYZOnlyPose2::write(std::ostream& os) const {
...
void EdgeSE3ProjectXYZOnlyPose2::linearizeOplus() {
    const VertexPose3DoF* v1 = static_cast<const VertexPose3DoF*>(_vertices[0]);
    Eigen::Vector3d xyz_trans = Rcw * Xw + v1->estimate();

    double x = xyz_trans[0];
    double y = xyz_trans[1];
    double z = xyz_trans[2];

    Eigen::Matrix<double,3,3> transderiv;
    transderiv << 1.f, 0.f, 0.f,
                    0.f, 1.f, 0.f,
                    0.f, 0.f, 1.f;

    _jacobianOplusXi = -pCamera->projectJac(xyz_trans) * transderiv;
}

Eigen::Vector3d EdgeStereoSE3ProjectXYZOnlyPose2::cam_project(const Eigen::Vector3d & trans_xyz) const{
...
bool EdgeStereoSE3ProjectXYZOnlyPose2::read(std::istream& is){
    for (int i=0; i<=3; i++){
        is >> _measurement[i];
    }
    for (int i=0; i<=2; i++)
...
bool EdgeStereoSE3ProjectXYZOnlyPose2::write(std::ostream& os) const {
...
void EdgeStereoSE3ProjectXYZOnlyPose2::linearizeOplus() {
    const VertexPose3DoF* v1 = static_cast<const VertexPose3DoF*>(_vertices[0]);
    Eigen::Vector3d xyz_trans = Rcw * Xw + v1->estimate();
