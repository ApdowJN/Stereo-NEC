...
void KeyFrame::AddImage(const cv::Mat &imLeft, const cv::Mat &imRight)
{
...
void KeyFrame::SetImuPoseVelocity(const Eigen::Matrix3f &Rwb, const Eigen::Vector3f &twb, const Eigen::Vector3f &Vwb)
{
    unique_lock<mutex> lock(mMutexPose);
    mVw = Vwb;
    mbHasVelocity = true;
...
bool KeyFrame::UnprojectStereoInBodyFrame(int i, Eigen::Vector3f &x3D)
{
    const float z = mvDepth[i];
    if(z>0)
    {
        ...
        Eigen::Matrix3f Rbc = mImuCalib.mTbc.rotationMatrix();
        Eigen::Vector3f tbc = mImuCalib.mTbc.translation();
        x3D = Rbc * x3Dc + tbc;
        return true;
    }
    else
